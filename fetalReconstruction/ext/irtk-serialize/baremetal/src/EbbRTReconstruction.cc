...
int MPMultiEbbCtrRoot::otherGatherVal(void) const {
  ...
  auto pair = boost::any_cast<
      std::pair<MPMultiEbbCtrRoot *,
                boost::container::flat_map<size_t, MPMultiEbbCtr *> > >(
      &accessor->second);
...
inline double M(double m, double _step) { return m * _step; }
...
void ScaleVolume() {
  ...
  for (inputIndex = 0; inputIndex < (unsigned int)_slices.size();
       inputIndex++) {
...
void ParallelAdaptiveRegularization1(vector<irtkRealImage> &_b,
...
void ParallelAdaptiveRegularization2(vector<irtkRealImage> &_b,
                                     vector<double> &_factor,
                                     irtkRealImage &_original) {
  int dx = _reconstructed.GetX();
  int dy = _reconstructed.GetY();
...
void AdaptiveRegularization(int iter, irtkRealImage &original) {
  ...
  if (_alpha * _lambda / (_delta * _delta) > 0.068) {
    ebbrt::kprintf("Warning: regularization might not have smoothing effect! "
                   "Ensure that alpha*lambda/delta^2 is below 0.068.\n");
  }
}
...
void NormaliseBias(int iter, int start, int end) {
  ...
  for (size_t i = 0; i < ncpus; i++) {
    ...
    ebbrt::event_manager->SpawnRemote(
        [theCpu, ncpus, &count, &context, i, diff, &bias]() {
          // get my cpu id
          ...
          irtkRealImage mbias;
          mbias.Initialize(_reconstructed.GetImageAttributes());
          mbias = 0;

          NormaliseBiasP(starte, ende, mbias);
...
void SuperresolutionP(int start, int end, irtkRealImage &addon,
        ...
        if (slice(i, j, 0) != -1) {
          ...
          for (int k = 0; k < n; k++) {
            ...
            _confidence_map(p.x, p.y, p.z) +=
                p.value * w(i, j, 0) * _slice_weight_cpu[inputIndex];
          }
        }
  } // end of loop for a slice inputIndex
}
...
void Superresolution(int iter, int start, int end) {
  ...
  for (size_t i = 0; i < ncpus; i++) {
    // spawn jobs on each core using SpawnRemote
    ebbrt::event_manager->SpawnRemote(
        [theCpu, ncpus, &count, &context, i, diff, &addon]() {
          // get my cpu id
          size_t mycpu = ebbrt::Cpu::GetMine();
          ...
          SuperresolutionP(starte, ende, maddon, m_confidence_map);
          ...
          // braces for scoping
          {
            ...
            std::lock_guard<ebbrt::SpinLock> l(spinlock);
            addon += maddon;
            _confidence_map += m_confidence_map;
          }
...
void Scale(int start, int end) {
  ...
  for (size_t i = 0; i < ncpus; i++) {
    // spawn jobs on each core using SpawnRemote
    ebbrt::event_manager->SpawnRemote(
        [theCpu, ncpus, &count, &context, i, diff]() {
          // get my cpu id
          size_t mycpu = ebbrt::Cpu::GetMine();
...
void EStepP(vector<double> &slice_potential, int start, int end) {
  for (size_t inputIndex = (size_t)start; inputIndex < (size_t)end;
       inputIndex++) {
    // read the current slice
    irtkRealImage slice = _slices[inputIndex];
...
void EStep(int start, int end) {
  ...
  for (size_t i = 0; i < ncpus; i++) {
    // spawn jobs on each core using SpawnRemote
    ebbrt::event_manager->SpawnRemote(
        [theCpu, ncpus, &count, &context, i, diff, &slice_potential]() {
          // get my cpu id
          size_t mycpu = ebbrt::Cpu::GetMine();
          ...
          EStepP(slice_potential, starte, ende);
  ...
  EStepP(slice_potential, start, end);
...
void SimulateSlicesP(int start, int end) {
  size_t inputIndex = 0;
  for (inputIndex = (size_t)start; inputIndex != (size_t)end; inputIndex++) {
    ...
    POINT3D p;
    for (unsigned int i = 0; i < (unsigned int)_slices[inputIndex].GetX(); i++)
...
void SimulateSlices(int start, int end) {
  ...
  // ebbrt::kprintf("SimulateSlicesP - number cpu: %d\n", ncpus);
  ...
  for (size_t i = 0; i < ncpus; i++) {
    ...
    ebbrt::event_manager->SpawnRemote(
        [theCpu, ncpus, &count, &context, i, diff]() {
          ...
          // ebbrt::kprintf("theCpu: %d, mycpu: %d, starte: %d, ende:%d\n",
          // theCpu,
          //		   mycpu, starte, ende);

          SimulateSlicesP(starte, ende);
  ...
  ebbrt::event_manager->SaveContext(context);
// ebbrt::kprintf("SimulateSlicesP - Context restored...\n");

#else

  SimulateSlicesP(start, end);
    ...
    for (i = 0; i < slice.GetX(); i++)
      for (j = 0; j < slice.GetY(); j++)
        if (slice(i, j, 0) != -1) {
          ...
          for (k = 0; k < n; k++) {
            p = _volcoeffs[inputIndex][i][j][k];
            recon(p.x, p.y, p.z) += p.value * slice(i, j, 0);
          }
        }
    voxel_num[inputIndex] = slice_vox_num;
  }
}

void GaussianReconstruction(int start, int end) {
  // clear _reconstructed image
  _reconstructed = 0;

#ifdef PARALLEL
  vector<int> voxel_num(end);
  int diff = end - start;
  size_t ncpus = ebbrt::Cpu::Count();
  ...
  for (size_t i = 0; i < ncpus; i++) {
    // spawn jobs on each core using SpawnRemote
    ebbrt::event_manager->SpawnRemote(
        [theCpu, ncpus, &count, &context, i, diff, &voxel_num]() {
          // get my cpu id
          size_t mycpu = ebbrt::Cpu::GetMine();
  ...
  for (int i = 0; i < (int)voxel_num.size(); i++)
...
void SliceToVolumeRegistrationP(int start, int end, irtkImageAttributes attr) {
  size_t inputIndex = 0;
  for (inputIndex = (size_t)start; inputIndex != (size_t)end; inputIndex++) {
...
void SliceToVolumeRegistration(int start, int end) {
  ...
  for (size_t i = 0; i < ncpus; i++) {
    // spawn jobs on each core using SpawnRemote
    ebbrt::event_manager->SpawnRemote(
        [theCpu, ncpus, &count, &context, i, diff, attr]() {
          // get my cpu id
          size_t mycpu = ebbrt::Cpu::GetMine();
          ...
          SliceToVolumeRegistrationP(starte, ende, attr);
...
void CoeffInitP(int start, int end) {
  size_t inputIndex = 0;
  for (inputIndex = (size_t)start; inputIndex != (size_t)end; inputIndex++) {
    bool slice_inside;
                        ...
                        for (n = nz; n <= nz + 1; n++)
                          if ((n >= 0) && (n < _reconstructed.GetZ())) {
                            ...
                            if ((aa < 0) || (aa >= dim) || (bb < 0) ||
                                (bb >= dim) || (cc < 0) || (cc >= dim)) {
                              ebbrt::kprintf(
                                  "Error while trying to populate tPSF\n");
                              exit(1);
                            } else
...
void CoeffInit(int start, int end) {
  ...
  // ebbrt::kprintf("CoeffInitP - number cpu: %d\n", ncpus);
  ...
  ebbrt::event_manager->SaveContext(context);
//    ebbrt::kprintf("CoeffInitP - Context restored...\n");
#else

  CoeffInitP(start, end);
